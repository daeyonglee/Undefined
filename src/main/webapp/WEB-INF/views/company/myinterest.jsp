...
<div class="content-area recent-property"
  ...
  <div class="container">
    <div class="row">
      ...
      <a href="dsinterest"><button type="button">DRESS</button></a>
      <a href="mkinterest"><button type="button">MAKEUP</button></a>
          ...
          <c:forEach items="${interlist}" var="studio" varStatus="status">
          <!-- 주소 자르기(split) -->
          <c:set value="${studio.sc_addr}" var="addr"/>
            ...
            <div class="col-md-4 p0">
              <div class="box-two proerty-item">
                <div class="item-thumb">
                  <a href="/company/compare?sc_no=${studio.sc_no }"><img
                    src="/user/mypage/imgview?imgview=${studio.sc_main_image}"></a>
                </div>
                <div class="item-entry overflow">
                  <h4>${studio.sc_nm }</h4>
                  <h4 class="heart"><a href="/user/mypage/myinterest/remove?sic_no=${studio.sic_no }" class="button" data-toggle="tooltip" title="삭제">♥</a></h4>
                  <div class="dot-hr"></div>
                  <span class="pull-left"><i class="pe-7s-map-marker strong"> </i>  <c:forEach var="word" items="${keywordArr}">${word} </c:forEach> </span><br>
                  <span class="pull-left"><i class="pe-7s-mail strong"> </i> ${studio.sc_email } </span><br>
                  <span class="pull-left"><i class="pe-7s-call strong"> </i> 010)${studio.sc_tel } </span><br>
 ...
 <!--  자르기4 결과 -->        ${split4 }
  ...
  <!-- other -->                     ${split4 }
          ...
          <!-- 위치랑 이름이랑 연결해서 찍어주는애 -->
              <input type="hidden" id="list"
               value= "
                 <c:forEach items="${interlist}" var="studio" varStatus="index">
                    ...
                    <c:set value="${fn:substringBefore(split4,'(').trim()}" var="split5"/>
                       ...
                       <c:choose>
                         <c:when test="${fn:contains(split4, '(')}" >
                             <c:choose>
                               <c:when test="${index.last}">${studio.sc_nm}:${split5}:${studio.sc_no }</c:when>
                                 <c:otherwise>${studio.sc_nm}:${split5}:${studio.sc_no}/</c:otherwise>
                               </c:choose>
                          </c:when>
                    
                     <c:otherwise>
                       <c:choose>
                          <c:when test="${index.last}">${studio.sc_nm}:${split4}:${studio.sc_no}</c:when>
                           <c:otherwise>${studio.sc_nm}:${split4}:${studio.sc_no}/</c:otherwise>
                    </c:choose>
                     </c:otherwise>
