  ...
  <div
      ...
      <button type="button" disabled>MAKEUP</button>
         ...
         <div class="page-subheader sorting pl0 pr-10"></div>
          ...
          <c:forEach items="${interlist}" var="makeup" varStatus="status">
          <!-- 주소 자르기(split) -->
          <c:set value="${makeup.mc_addr}" var="addr"/>
            ...
            <div class="col-md-4 p0">
              <div class="box-two proerty-item">
                ...
                <div class="item-entry overflow">
                  <h4>${makeup.mc_nm }</h4>
                   <h4 class="heart"><a href="/user/mypage/mkinterest/remove?mic_no=${makeup.mic_no }" class="button" data-toggle="tooltip" title="삭제">♥</a></h4>
                  <div class="dot-hr"></div>
                  <span class="pull-left"><i class="pe-7s-map-marker strong"> </i>  <c:forEach var="word" items="${keywordArr}">${word} </c:forEach> </span><br>
                  <span class="pull-left"><i class="pe-7s-mail strong"> </i> ${makeup.mc_email } </span><br>
                  <span class="pull-left"><i class="pe-7s-call strong"> </i> 010)${makeup.mc_tel } </span><br>
    ...
    <!-- makeup.mc_addr:makeup.mc_nm, -->
                            
                            <c:forEach items="${list}" var="makeup" varStatus="index">
 ...
 <!--  자르기4 결과 -->        ${split4 }
  ...
  <!-- other -->                     ${split4 }
          ...
          <!-- 위치랑 이름이랑 연결해서 찍어주는애 -->
              <input type="hidden" id="list"
               value= "
                 <c:forEach items="${interlist}" var="makeup" varStatus="index">
                    ...
                    <c:set value="${fn:substringBefore(split4,'(').trim()}" var="split5"/>
                       ...
                       <c:choose>
                         <c:when test="${fn:contains(split4, '(')}" >
                             <c:choose>
                               <c:when test="${index.last}">${makeup.mc_nm}:${split5}:${makeup.mc_no }</c:when>
                                 <c:otherwise>${makeup.mc_nm}:${split5}:${makeup.mc_no}/</c:otherwise>
                               </c:choose>
                          </c:when>
                    
                     <c:otherwise>
                       <c:choose>
                          <c:when test="${index.last}">${makeup.mc_nm}:${split4}:${makeup.mc_no}</c:when>
                           <c:otherwise>${makeup.mc_nm}:${split4}:${makeup.mc_no}/</c:otherwise>
                    </c:choose>
                     </c:otherwise>
